...
namespace {
    ...
    // Простое URL-декодирование (ограниченно к q-параметру)
    ...
    renderer_(renderer)
{
    ...
    if (ec) return;
    // Исправление: максимальная очередь ожидания подключений
    acceptor_.listen(boost::asio::socket_base::max_listen_connections, ec);
    // При необходимости обрабатывать ec
}
...
void HttpServerBeast::run()
{
    ...
    for (;;) {
        ...
        // Правильное извлечение target без to_string()
